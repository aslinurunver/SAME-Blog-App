...
const ArticleCard = ({ className, post }) => {
  return (
    <div className={`overflow-hidden rounded-xl shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] ${className}`}>
      ...
      <div className="p-5">
        <Link to={`/blog/${post.slug}`}>
          <h2 className="text-xl font-bold text-dark-soft md:text-2xl lg:text-[28px]">{post.title}</h2>
        ...
        <div className="mt-6 flex flex-nowrap items-center justify-between">
          <div className="flex items-center gap-x-2 md:gap-x-2.5">
            <img
              src={
                post?.user?.avatar !== null ? stables.UPLOAD_FOLDER_BASE_URL + post?.user?.avatar : images.userPostImage
              }
              alt="Profilepicture"
              className="h-9 w-9 md:h-10 md:w-10"
            />
            <div className="flex flex-col">
              <h4 className="text-sm font-bold italic text-dark-soft md:text-base">{post.user?.name}</h4>
